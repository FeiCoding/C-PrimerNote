...
15. 只有空指针在条件判断中为false，其他值得指针都为true。两个指针可以使用“==”和"!="来判断他们存放的地址是否相同。
16. void指针是一种特殊的指针，可用于存放任意对象的地址。void\*存放地址，但我们不了解该指针存放地址的类型是什么。
...
5. 相反的，允许常量引用绑定一个非常量的对象或字面值。**常量引用可以引用一个字面值。**
...
7. 同样的，常量的地址只能被指向常量的指针（pointer to const， 将const放在类型前面，例如const int _ptr）存储，但\*指向常量的指针也可以指向一个非常量对象_，只是该指针不能修改其内容。
...
14. 执行拷贝操作并不会改变被拷贝对象的值，所以考入和考出的对象对于顶层const是否是常量都没什么影响。然而对于底层const，考入考出的拷贝对象必须具有相同的底层const
