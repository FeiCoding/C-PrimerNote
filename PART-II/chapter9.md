...
1. begin和end函数是重载过得，也就是说，实际上有两个名为begin的成员，一个是const成员，返回容器的const_iterator类型，另一个是非常量成员，返回容器的iterator类型的。当我们对一个非常量对象对用这些成员函数时，得到的是返回iterator版本，而常量对象则返回const_iterator。c++11引入了cbegin和cend。

1. 当不需要写访问时，应当使用cbegin和cend函数，用以支持搭配auto使用的。
...
1. **当将一个容器初始化为另一个容器的拷贝时，则必须两个容器的类型和元素类型完全相同。**
...
1. **当定义一个array时，除了制定元素类型，还要指定容器大小：**
...
1. **虽然不能对内置数组类型进行拷贝或对象赋值操作，但array并无此限制：**
...
1. 标准库array类型允许赋值，赋值号左右两边的运算对象必须有相同的类型：
