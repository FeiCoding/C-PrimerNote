...
1. 如果用一个对象去初始化另一个对象，则他们是不是const都无关紧要：

    ```C++
    const int j; // 不合法，const必须初始化
    int i = 42;
    const int ci = i; // 合法
    int j = ci; // 合法
    ```
...
1. 同样的，常量的地址只能被指向常量的指针（pointer to const）存储（将const放在类型前面，例如const int *ptr），但指向常量的指针也可以指向一个非常量对象，只是该指针不能修改其内容。

1. 被指向常量的指针或是常量引用绑定的非常量对象，虽然无法通过指针或者引用改变值，但可以通过其他方式改变值。

1. 常量指针（将const放在*的后边，例如int *const ptr）**必须初始化**，而且一旦初始化完成它所存储的地址值就无法改变了。然而在该地址上存储的值却是依赖于其类型，如果其不是const类型，也可以改变，只是改变的结果依然存在该地址，所以此时常量指针所存储的值仍然是该地址而没有发生改变。
